...
use std::{cmp::min, error::Error, fmt::Display, marker::PhantomData, time::Instant};
...
// TODO: Is there some way to validate in place without creating new list?
...
impl<V: Validate> Validate for IndexMap<String, V> {
    ...
    fn validate<T: Validator>(
        ...
        let res = self
            .into_iter()
            .map(
                |(key, value)| match item_validator.validate_item(value_schema, value) {
                    Ok(val) => Ok((key, val)),
...
mod tests {
    ...
    use crate::{
        ...
        schema::{LIST_DOCUMENT_SCHEMA, Schema, SchemaShape, ShapeId},
        serde::validation::SmithyConstraints::Required,
        traits,
    };
    ...
    fn checks_list_too_long() {
        ...
        let expected =
            "Size: 3 does not conform to @length constraint. Expected between 1 and 2.".to_string();
        assert_eq!(expected, format!("{}", errors.get(0).unwrap().code));
    }

    //////////////////////////////////////////////////////////////////////////////////
    // Example Builder Validation
    //////////////////////////////////////////////////////////////////////////////////
    lazy_schema!(
        SCHEMA_B,
        Schema::structure_builder(ShapeId::from("com.example#ShapeB"), traits![]),
        (MEMBER_AB, "a", STRING, traits![RequiredTrait::new()])
    );

    lazy_schema!(
        SCHEMA_A,
        Schema::structure_builder(ShapeId::from("com.example#ShapeA"), traits![]),
        (MEMBER_A, "a", STRING, traits![RequiredTrait::new()]),
        (MEMBER_B, "b", STRING, traits![]),
        (MEMBER_C, "c", SCHEMA_B, traits![RequiredTrait::new()])
    );
    struct A {
    ...
    struct ABuilder {
        field: Option<String>,
        field_opt: Option<i32>,
        // TODO: We should also all pre-built structs here. Might need an enum type to handle.
        b: Option<BBuilder>,
    }

    // This builder implementation represents what we would expect to be
    // generated by a code generator or proc macro.
    impl ABuilder {
        #[allow(clippy::unwrap_used, clippy::unnecessary_literal_unwrap)]
        fn build(self) -> Result<A, ValidationErrors> {
            let mut errors = ValidationErrors::new();
            let field = DefaultValidator
                .validate_required(&MEMBER_A, self.field)
                .map_err(|e| errors.extend(e));
            let field_opt = DefaultValidator
                .validate_optional(&MEMBER_B, self.field_opt)
                .map_err(|e| errors.extend(e));
            let field_b = DefaultValidator
                .validate_required(&MEMBER_C, self.b)
                .map_err(|e| errors.extend(e));
            if !errors.is_empty() {
                Err(errors)
            } else {
                // Unwrapping here is safe because any errors would result in err above
                // being returned.
                Ok(A {
                    field: field.unwrap(),
                    field_opt: field_opt.unwrap(),
                    nested: field_b.unwrap(),
                })
            }
        }
    }

    impl Validate for ABuilder {
        type Value = A;
    ...
    impl BBuilder {
        #[allow(clippy::unwrap_used, clippy::unnecessary_literal_unwrap)]
        fn build(self) -> Result<B, ValidationErrors> {
            let mut errors = ValidationErrors::new();
            let field = DefaultValidator
                .validate_required(&MEMBER_AB, self.field)
                .map_err(|e| errors.extend(e));
            if !errors.is_empty() {
                Err(errors)
            } else {
    ...
    #[test]
    fn build_when_valid() {
        let b_builder = BBuilder {
            field: Some("String-B".to_string()),
        };
        let a_builder = ABuilder {
            field: Some("String-A".to_string()),
            field_opt: None,
            b: Some(b_builder),
        };
        let a = a_builder.build().unwrap();
        assert_eq!(a.field, "String-A".to_string());
    }

    #[test]
    fn fails_when_invalid() {
        let b_builder = BBuilder {
            field: Some("String-B".to_string()),
        };
        let a_builder = ABuilder {
        ...
        if let Some(err) = a.err() {
            ...
            let inner = err.errors.get(0).unwrap();
            assert_eq!(&inner.path, &*MEMBER_A.id());
        }
    }

    #[test]
    fn bubbles_up_errors() {
        let b_builder = BBuilder { field: None };
        let a_builder = ABuilder {
            field: Some("String-A".to_string()),
            field_opt: None,
            b: Some(b_builder),
        ...
        if let Some(err) = a.err() {
            assert_eq!(err.errors.len(), 1);
            let inner = err.errors.get(0).unwrap();
            assert_eq!(&inner.path, &*MEMBER_AB.id());
            println!("{err}")
        }
    }
}
